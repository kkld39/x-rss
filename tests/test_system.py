...
def post(id="2103031729787445314", **kwargs):
    return replace(Post(id, "日本語 & <script>alert(1)</script>\n😀", NOW,
                        "Mazda_PR", "表示名", ["https://pbs.twimg.com/a.jpg?x=1&y=2"]), **kwargs)
...
def tweet(**kwargs):
    ...
    value = {"id": 0, "id_str": "2103031729787445314", "full_text": "本文 &amp; 🐈",
             ...
             "user": {"screen_name": "Mazda_PR", "name": "表示名"},
...
class ParserTests(unittest.TestCase):
    def test_live_shaped_normal_reply_repost_quote(self):
        ...
        a, b, c, d = parse_timeline(document([normal, reply, repost, quote]), "Mazda_PR")
...
class RssTests(unittest.TestCase):
    def test_rss_unicode_guid_dates_author_images_and_escaping(self):
        ...
        root = ET.fromstring(rss("Mazda_PR", [value], "https://example.github.io/repo", NOW))
    ...
    def test_unsafe_image_not_embedded(self):
        xml = rss("Mazda_PR", [post(images=["javascript:alert(1)"])], "https://example.org", NOW)
...
class RunnerTests(unittest.TestCase):
    ...
    def seed(self):
        self.run_app(Config(["Mazda_PR"]), FakeSource({"Mazda_PR": [post()]}))
        return (self.data / "Mazda_PR.json").read_bytes(), (self.output / "feeds/Mazda_PR.xml").read_bytes()
    ...
    def test_partial_failure_preserves_old_files_and_continues(self):
        ...
        self.assertEqual(state["mazda_pr"]["last_success"], NOW)
        self.assertFalse(state["mazda_pr"]["ok"])

    def test_all_failure_returns_nonzero_and_updates_status_page(self):
        history, feed = self.seed()
        result = self.run_app(Config(["Mazda_PR"]), FakeSource({"Mazda_PR": FetchError("HTTP 403")}))
    ...
    def test_first_failure_does_not_create_empty_feed_or_history(self):
        self.assertEqual(self.run_app(Config(["Mazda_PR"]), FakeSource({"Mazda_PR": []})), 1)
        self.assertFalse((self.data / "Mazda_PR.json").exists())
    ...
    def test_filter_changes_apply_to_history_and_quotes_remain(self):
        ...
        source = FakeSource({"Mazda_PR": incoming})
        self.run_app(Config(["Mazda_PR"]), source)
        self.assertEqual(len(load_posts(self.data / "Mazda_PR.json")), 4)
        path = self.output / "feeds/Mazda_PR.xml"
        self.assertEqual(len(ET.parse(path).findall("channel/item")), 2)
        self.run_app(Config(["Mazda_PR"], include_reposts=True, include_replies=True),
                     FakeSource({"Mazda_PR": [post("1")]}))
        self.assertEqual(len(ET.parse(path).findall("channel/item")), 4)
        self.assertEqual(json.loads((self.data / "_meta/status.json").read_text())["mazda_pr"]["new_count"], 0)

    def test_corrupt_history_is_not_overwritten(self):
        self.data.mkdir()
        path = self.data / "Mazda_PR.json"
        path.write_text("broken", encoding="utf-8")
        self.assertEqual(self.run_app(Config(["Mazda_PR"]), FakeSource({"Mazda_PR": [post()]})), 1)
...
class ConfigTests(unittest.TestCase):
    def test_invalid_config(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for content in ("accounts: []", "accounts: [../bad]", "accounts: [Mazda_PR, mazda_pr]",
                            'accounts: [Mazda_PR]\ninclude_replies: "false"',
                            "accounts: [Mazda_PR]\nmax_posts: 2", "accounts: [Mazda_PR]\nwrong: true"):
